...
const GroupChat = () => {
  ...
  useEffect(() => {
    const getChats = async () => {
      ...
      const chatSnap = await get(
        child(chatRef, `groups/${activeGroup.groupId}`)
      );
      try {
        if (chatSnap.exists()) {
  ...
  const handleSend = async () => {
    ...
    await update(
      ref(chatDb, "groups/" + activeGroup.groupId + "/" + currentDate),
      {
        message,
          ...
          <ScrollArea className="flex-grow overflow-y-auto">
            {!chats.length == 0 ? (
              chats.map((chat, index) => {
                const dateTime = new Date(chat.dateTime).toLocaleString(
                  "en-us",
                  {
                    hour: "numeric",
                    minute: "numeric",
                ...
                return (
                  <div key={index} ref={scrollAreaRef}>
                    {isNewDate && (
                      <div className="mt-1 bg-green-700 w-fit p-1 rounded-lg relative left-[50%] right-[50%]">
                        {dateString}
                      </div>
                    )}
